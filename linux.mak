...
.PHONY: clean
clean:
	if test -d "$(LINUX_SRC)"; then $(MAKE) -j6 -C $(LINUX_SRC) clean ; fi
...
$(LINUX_IMAGE_BIN): $(LINUX_TC_DIR) $(LINUX_SRC)
	$(MAKE) -j6 -C $(LINUX_SRC) $(LINUX_CONFIG)
	$(MAKE) -j6 -C $(LINUX_SRC) $(LINUX_IMAGE_FILE)
	$(MAKE) -j6 -C $(LINUX_SRC) dtbs
	$(MAKE) -j6 -C $(LINUX_SRC) modules
	touch $@
...
$(MODS_DIR): $(LINUX_IMAGE_BIN)
	...
	mkdir -p "$@.tmp"
	$(MAKE) -j6 -C $(LINUX_SRC) modules
	$(MAKE) -j6 -C $(LINUX_SRC) INSTALL_MOD_PATH=$(abspath $(MODS_DIR).tmp) modules_install
	mv "$@.tmp" $@
	touch $@
